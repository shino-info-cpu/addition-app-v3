(function (global) {
  ...
  additions: [
    ...
    },
    {
      ...
      serviceDecisionInclude: ["医療関連", "障害福祉以外の福祉サービス"],
    ...
    },
    {
      ...
      requiredAnswers: { requiredInfoReceived: "受けた" },
      ...
      postCheckRules: [
        ...
        { code: "monthly_limit_per_client", limit: 1, additionCodes: ["mededu", "mededu_interview", "mededu_meeting"], recordActionTypes: ["面談", "会議"], label: "同月1回まで" },
        { code: "exclusive_with_addition_codes", additionCodes: ["conference"], label: "担当者会議加算との併算定不可" },
      ],
      postCheck: "面談相手と受けた情報を記録する。同月1回までと担当者会議加算との重複を後段で確認する。",
    },
    {
      ...
      postCheck: "会議参加先と得た情報を記録する。同月1回までと担当者会議加算との重複を後段で確認する。",
    },
    {
    ...
    },
    {
      ...
      serviceDecisionInclude: ["医療関連", "障害福祉サービス", "障害福祉以外の福祉サービス"],
      placeTypes: ["自事業所内", "外出先"],
      actionTypes: ["面談", "退院前面談"],
      requiredAnswers: { serviceUseStartMonth: "開始月である", requiredInfoReceived: "受けた" },
      postCheckRules: [
        { code: "blocked_if_answer_true", answerKey: "dischargeInpatientPeriodCount", blockedValue: "4回目以上・不明", label: "同一の入所・入院期間中は3回まで" },
        { code: "blocked_if_answer_true", answerKey: "initialAdditionPlanned", blockedValue: "初回加算あり", label: "初回加算算定時は不可" },
      ],
      postCheck: "当該施設職員との面談内容と受けた情報を記録する。",
    },
    {
    ...
    },
    {
      ...
      conditionalRequiredAnswers: [
        {
          whenOrganizationTypes: ["企業", "障害者就業・生活支援センター"],
  ...
  reportRecords: [
    { recordId: "r1", targetMonth: "2026-03", performedAt: "2026-03-14 09:05", clientId: "1001", organizationId: "21", serviceId: "301", staffId: "501", additionCode: "mededu", actionType: "情報共有", finalStatus: "自動確定", postCheckStatus: "ok", postCheckSummary: "同グループ月1回まで（病院・訪看・薬局グループ）。今月既存0件で範囲内です。", evaluatedAt: "2026-03-14 09:20", rationale: "病院グループ / 計画作成月 / 自事業所内 / 情報共有", savedNote: "病院と支援方針を共有し、今後の通院支援計画を確認した。" },
    { recordId: "r2", targetMonth: "2026-03", performedAt: "2026-03-14 09:50", clientId: "1002", organizationId: "11", serviceId: "203", staffId: "502", additionCode: "intensive", actionType: "訪問", finalStatus: "要確認", postCheckStatus: "review", postCheckSummary: "同月2回以上の訪問が必要。今回を含めて1回です。", evaluatedAt: "2026-03-14 10:05", rationale: "福祉サービス等提供機関 / それ以外 / 外出先 / 訪問", savedNote: "生活介護の現場を訪問し、利用状況と支援上の課題を確認した。" },
    { recordId: "r3", targetMonth: "2026-03", performedAt: "2026-03-14 11:10", clientId: "1003", organizationId: "10", serviceId: "201", staffId: "501", additionCode: "conference", actionType: "担当者会議開催", finalStatus: "自動確定", postCheckStatus: "ok", postCheckSummary: "同月1回まで。今月既存0件で範囲内です。 / 医保教（面談・会議）との併算定不可。今月の併算定不可記録は見つかっていません。", evaluatedAt: "2026-03-14 11:42", rationale: "福祉サービス等提供機関 / モニタリング月 / 自事業所内 / 担当者会議開催", savedNote: "モニタリングに当たり担当者会議を開催し、関係機関で支援方針を共有した。" },
    { recordId: "r4", targetMonth: "2026-02", performedAt: "2026-02-28 15:30", clientId: "1001", organizationId: "21", serviceId: "301", staffId: "501", additionCode: "discharge", actionType: "面談", finalStatus: "自動確定", postCheckStatus: "ok", postCheckSummary: "当該施設職員との面談内容と受けた情報を記録する。", evaluatedAt: "2026-02-28 16:18", rationale: "病院 / それ以外 / 外出先 / 面談", savedNote: "当該施設の職員と面談し、退院後支援に必要な情報を受けて調整した。" },
    { recordId: "r5", targetMonth: "2026-03", performedAt: "2026-03-13 13:25", clientId: "1001", organizationId: "11", serviceId: "202", staffId: "503", additionCode: "mededu_interview", actionType: "面談", finalStatus: "自動確定", postCheckStatus: "ok", postCheckSummary: "同月1回まで。今月既存0件で範囲内です。", evaluatedAt: "2026-03-13 14:00", rationale: "福祉サービス等提供機関 / モニタリング月 / 外出先 / 面談", savedNote: "福祉サービス等提供機関の担当者と面談し、モニタリングに必要な情報を受けた。" },
  ...
  },
  {
    ...
    visibleWhen() {
      return shouldShowCandidateFactQuestion("requiredInfoReceived");
    },
    options: [
      { value: "受けた", note: "加算判定に必要な情報の提供を受けた" },
      { value: "受けていない", note: "面談や調整はしたが、必要な情報の提供は受けていない" },
    ],
  },
  {
  ...
  },
  {
    ...
    visibleWhen({ candidates }) {
      return candidates.some((candidate) => candidateRequiresAnswer(candidate, "dischargeInpatientPeriodCount"));
    },
    options: [
      { value: "1回目", note: "この入所・入院期間で最初の調整" },
      { value: "2回目", note: "この入所・入院期間ですでに1回記録がある" },
      { value: "3回目", note: "この入所・入院期間ですでに2回記録がある" },
      { value: "4回目以上・不明", note: "4回目以上、または回数を確定できない" },
    ],
  },
  {
  ...
  },
  {
    ...
    visibleWhen() {
      const candidates = getJudgementCandidatesExcludingAnswers(["serviceUseStartMonth"]);
      return candidates.length === 1 && shouldShowCandidateFactQuestion("serviceUseStartMonth");
    },
    options: [
  ...
  global.__KASAN_PROTOTYPE_RULE_SOURCE__ = source;
